...
export default function App() {
  ...
  // Real-time polling: refresh bookings every 5 seconds for chat sync
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const newBookings = await loadList(BOOKINGS_KEY);
  ...
  async function submitOrgRequest(data) {
    ...
    const next = [org, ...orgs];
    setOrgs(next); await saveList(ORGS_KEY, next);
    showToast('Гэрээт үйлчилгээний хүсэлт илгээгдлээ. Бид удахгүй холбогдоно.');
  }
  async function approveOrg(id) {
    const next = orgs.map(o => o.id === id ? { ...o, status: 'active', paid: true, activatedAt: Date.now() } : o);
    setOrgs(next); await saveList(ORGS_KEY, next);
...
// ============ HOME ============
function Home({ onPlaceCall, onMaterials, onTechRegister, onTrack, onAdmin, onHelp, onFeedback, onOrgs }) {
  return (
    ...
    <div className="uz-settings">
      ...
      <div className="uz-settings-form" style={{ marginBottom: 28 }}>
        <div className="uz-field"><label>Сарын суурь хураамж (₮)</label>
          <input type="number" value={orgFee} onChange={e => setOrgFee(e.target.value)} placeholder="300000" /></div>
      </div>
...
/* DOWN ARROWS */
.uz-home-sub-row { display:flex; align-items:center; justify-content:center; gap:18px; flex-wrap:wrap; }
.uz-arrow-down { font-size:32px; color:var(--blue); animation:uzBounce 1.5s ease-in-out infinite; line-height:1; }
