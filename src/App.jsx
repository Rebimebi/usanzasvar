...
export default function App() {
  ...
  // Real-time polling: refresh all key data every 2 seconds
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const [newBookings, newTechs, newFeedback, newOrgs, newCustomers] = await Promise.all([
  ...
  useEffect(() => {
    (async () => {
      try {
        const session = await loadObj(SESSION_KEY);
        if (!session || !session.role) return;
        if (Date.now() - session.savedAt > SESSION_MAX_AGE_MS) {
          await saveObj(SESSION_KEY, null);
          return;
        }
  ...
  async function saveSession(role, phone) {
    try { await saveObj(SESSION_KEY, { role, phone, savedAt: Date.now() }); } catch (e) {}
  }
  async function clearSession() {
    try { await saveObj(SESSION_KEY, null); } catch (e) {}
  }
  ...
  // UNREAD COUNT - title flashing when away
  ...
  async function sendBroadcast(title, message, audience) {
    const broadcast = {
      id: uid(),
      title: title.trim(),
    ...
    const next = [broadcast, ...broadcasts];
    setBroadcasts(next); await saveList(BROADCASTS_KEY, next);
...
// ============ HOME ============
function Home({ onPlaceCall, onMaterials, onTechRegister, onTrack, onAdmin, onHelp, onFeedback, onOrgs, onReviews, latestBroadcast, onDismissBroadcast }) {
  return (
...
function MyBookings({ bookings, hasPhone, onLookup, onNew, onRate, onDispute, onSendMessage, onBack, myCoins, settings }) {
  ...
  return (<div className="uz-page"><button className="uz-back" onClick={onBack}>← Нүүр буцах</button><h1 className="uz-page-title">Миний дуудлага</h1>
    {(() => {
      const completedCount = bookings.filter(b => b.completed && !b.disputed).length;
      const tier = getTier(completedCount);
      const nextTier = getNextTier(completedCount);
      const left = nextTier ? nextTier.minCount - completedCount : 0;
      return (
        <div className="uz-tier-card" style={{ background: `linear-gradient(135deg, ${tier.color}, ${tier.color}cc)` }}>
          <div className="uz-tier-emoji">{tier.emoji}</div>
          <div className="uz-tier-body">
            <div className="uz-tier-name">{tier.name} <span className="uz-tier-badge">{tier.badge}</span></div>
            <div className="uz-tier-sub">{completedCount} амжилттай дуудлага {nextTier ? `· ${left} дуудлагын дараа ${nextTier.name} болно ${nextTier.emoji}` : '· Та хамгийн дээд түвшинд хүрсэн!'}</div>
          </div>
        </div>
      );
    })()}
    ...
    <div className="uz-bookings">{bookings.map(b => <CustomerBookingCard key={b.id} b={b} onRate={onRate} onDispute={onDispute} onSendMessage={onSendMessage} settings={settings} />)}</div></div>);
}

function CustomerBookingCard({ b, onRate, onDispute, onSendMessage, settings }) {
        ...
        {b.paymentStatus === 'pending' && (
          <div className="uz-pay-pending-box">
            <div className="uz-pay-pending-warn">
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: '#7a5a00' }}>⚠ Анхааруулга: Дуудлага зөвхөн төлбөр баталгаажсаны дараа идэвхжинэ. Сантехникч таны төлбөр орсон гэж баталгаажсаны дараа л дуудлага хүлээж авна.</span>
            </div>
            {settings && settings.accountNumber && (
              <div className="uz-pay-bank-info">
                ...
                <div className="uz-pay-bank-row"><span>Данс:</span><b className="uz-pay-bank-acct">{settings.accountNumber}</b><CopyButton value={settings.accountNumber} /></div>
                <div className="uz-pay-bank-row"><span>Эзэмшигч:</span><b>{settings.accountHolder || ''}</b></div>
                <div className="uz-pay-bank-row uz-pay-bank-amt"><span>Дүн:</span><b>₮{fmt((settings.calloutFee || CALLOUT_FEE))}</b></div>
                <div className="uz-pay-bank-row"><span>Гүйлгээний утга:</span><b className="uz-pay-bank-acct">{b.code}</b><CopyButton value={b.code} /></div>
  ...
  return (
    <div className="uz-page" style={{ maxWidth: 1100 }}>
      ...
      <div className="uz-dash-tabs">
        ...
        <button className={`uz-dash-tab ${tab === 'broadcasts' ? 'active' : ''}`} onClick={() => setTab('broadcasts')}>📢 Зар сурталчилгаа</button>
  ...
  return (
    <div>
      <div className="uz-settings-form" style={{ marginBottom: 20 }}>
        ...
        <p className="uz-settings-sub" style={{ marginBottom: 14 }}>Бүх хэрэглэгчдийн аппын дэлгэцэнд тэр даруй харагдана. Push notification ч ирнэ.</p>
            ...
            <div key={b.id} className="uz-booking-card">
              <div className="uz-bc-body" style={{ flex: 1 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
                  ...
                  <span className="uz-bc-code">{b.audience === 'techs' ? '👷 Сантехникч' : b.audience === 'customers' ? '👤 Үйлчлүүлэгч' : '🌐 Бүгд'}</span>
                </div>
...
.uz-pay-bank-row { display:flex; align-items:center; gap:8px; padding:8px 0; border-bottom:1px solid var(--line); font-size:13px; flex-wrap:wrap; }
.uz-pay-bank-row:last-child { border-bottom:none; }
.uz-pay-bank-row span { color:var(--ink-soft); min-width:90px; }
.uz-pay-bank-row b { color:var(--ink); flex:1; word-break:break-all; }
.uz-pay-bank-acct { font-family:monospace; font-size:15px !important; letter-spacing:0.5px; }
.uz-pay-bank-amt { background:#e6f7ef; margin:6px -6px 6px; padding:8px 12px; border-radius:8px; border:none; }
.uz-pay-bank-amt b { color:#0d8a56; font-size:18px !important; }
