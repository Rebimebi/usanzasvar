...
const VISITS_KEY = 'usanzasvar:visits';
const VISITOR_ID_KEY = 'usanzasvar:visitor-id';
...
async function trackVisit() {
  ...
  await atomicUpdateObj(VISITS_KEY, (current) => {
    const data = current || {
      total: 0,
      uniqueVisitors: [],
      byDay: {},
    ...
    // Daily counts (keep last 30 days only)
    if (!data.byDay) data.byDay = {};
    data.byDay[today] = (data.byDay[today] || 0) + 1;
    
    // Trim old days (>30 days)
    const days = Object.keys(data.byDay).sort().reverse();
    if (days.length > 30) {
      const keepDays = days.slice(0, 30);
      const newByDay = {};
      keepDays.forEach(d => { newByDay[d] = data.byDay[d]; });
      data.byDay = newByDay;
    }
...
export default function App() {
  ...
  // Track visit on app load (once per session)
  useEffect(() => {
    const sessionKey = 'usanzasvar:visited-this-session';
    if (!sessionStorage.getItem(sessionKey)) {
      trackVisit().catch(err => console.log('Visit tracking failed:', err));
  ...
  return (
    <div className="uz-page" style={{ maxWidth: 1100 }}>
      ...
      <div className="uz-dash-tabs">
        ...
        <button className={`uz-dash-tab ${tab === 'stats' ? 'active' : ''}`} onClick={() => setTab('stats')}>📊 Статистик</button>
      ...
      {/* STATS */}
      {tab === 'stats' && <StatsPanel techs={techs} bookings={bookings} customers={customers} />}
...
// ============ STATS PANEL ============
function StatsPanel({ techs, bookings, customers }) {
  const [visits, setVisits] = useState(null);
  const [loading, setLoading] = useState(true);
  
  useEffect(() => {
    (async () => {
      const data = await loadObj(VISITS_KEY);
      setVisits(data);
      setLoading(false);
    })();
  }, []);
  
  if (loading) return <div className="uz-empty-sm">Уншиж байна...</div>;
  
  const totalVisits = visits?.total || 0;
  const uniqueVisitors = visits?.uniqueVisitors?.length || 0;
  const byDay = visits?.byDay || {};
  const lastVisit = visits?.lastVisit;
  
  // Calculate today, yesterday, this week, this month
  ...
  const weekCount = Object.entries(byDay)
  ...
  // Booking stats
  const completedBookings = bookings.filter(b => b.completed).length;
  const pendingBookings = bookings.filter(b => !b.accepted && !b.completed).length;
  const activeBookings = bookings.filter(b => b.accepted && !b.completed).length;
  const conversionRate = totalVisits > 0 ? ((bookings.length / totalVisits) * 100).toFixed(1) : '0';
  
  return (
    <div className="uz-stats-panel">
      {/* TOP STATS CARDS */}
      <div className="uz-stats-grid">
        <div className="uz-stat-card uz-stat-blue">
          <div className="uz-stat-icon">👁</div>
          <div className="uz-stat-value">{totalVisits.toLocaleString()}</div>
          <div className="uz-stat-label">Нийт зочлолт</div>
        </div>
        <div className="uz-stat-card uz-stat-green">
          <div className="uz-stat-icon">👤</div>
          <div className="uz-stat-value">{uniqueVisitors.toLocaleString()}</div>
          <div className="uz-stat-label">Хувийн зочин</div>
        </div>
        <div className="uz-stat-card uz-stat-gold">
          <div className="uz-stat-icon">📅</div>
          <div className="uz-stat-value">{todayCount}</div>
          <div className="uz-stat-label">Өнөөдөр</div>
        </div>
        <div className="uz-stat-card uz-stat-purple">
          <div className="uz-stat-icon">📊</div>
          <div className="uz-stat-value">{weekCount}</div>
          <div className="uz-stat-label">7 хоног</div>
        </div>
      </div>

      {/* COMPARISON */}
      <div className="uz-stats-compare">
        <div className="uz-stats-compare-item">
          <div className="uz-stats-compare-label">Өчигдөр</div>
          <div className="uz-stats-compare-value">{yesterdayCount}</div>
        </div>
        <div className="uz-stats-compare-arrow">
          {todayCount > yesterdayCount ? '📈' : todayCount < yesterdayCount ? '📉' : '➡️'}
        </div>
        <div className="uz-stats-compare-item">
          <div className="uz-stats-compare-label">Өнөөдөр</div>
          <div className="uz-stats-compare-value" style={{ color: 'var(--blue)' }}>{todayCount}</div>
        </div>
        <div className="uz-stats-compare-diff" style={{
          color: todayCount > yesterdayCount ? '#16a085' : todayCount < yesterdayCount ? '#e2483f' : '#666',
        }}>
      ...
      {/* DAILY CHART */}
      {sortedDays.length > 0 && (
        <div className="uz-stats-section">
          <h3 className="uz-stats-section-title">📊 Сүүлийн 14 хоног</h3>
          <div className="uz-stats-chart">
            {sortedDays.map(([day, count]) => {
              const heightPct = (count / maxCount) * 100;
              const isToday = day === today;
              const shortDay = day.slice(5); // MM-DD
              return (
                <div key={day} className="uz-stats-bar-wrap">
                  <div className="uz-stats-bar-count">{count}</div>
                  <div 
                    className={`uz-stats-bar ${isToday ? 'uz-stats-bar-today' : ''}`}
                    style={{ height: `${heightPct}%` }}
                  />
                  <div className="uz-stats-bar-label">{shortDay}</div>
                </div>
              );
            })}
          </div>
      ...
      {/* BUSINESS METRICS */}
      <div className="uz-stats-section">
        <h3 className="uz-stats-section-title">💼 Бизнес метрикс</h3>
        <div className="uz-stats-metrics">
          <div className="uz-stats-metric">
            <div className="uz-stats-metric-label">Дуудлага</div>
            <div className="uz-stats-metric-value">{bookings.length}</div>
            <div className="uz-stats-metric-sub">{completedBookings} дууссан · {activeBookings} идэвхтэй · {pendingBookings} хүлээгдэж буй</div>
          </div>
          <div className="uz-stats-metric">
            <div className="uz-stats-metric-label">Сантехникч</div>
            <div className="uz-stats-metric-value">{techs.length}</div>
            <div className="uz-stats-metric-sub">{techs.filter(t => t.approved !== false).length} зөвшөөрөгдсөн</div>
          </div>
          <div className="uz-stats-metric">
            <div className="uz-stats-metric-label">Хэрэглэгч</div>
            <div className="uz-stats-metric-value">{customers.length}</div>
            <div className="uz-stats-metric-sub">Хадгалагдсан</div>
          </div>
          <div className="uz-stats-metric">
            <div className="uz-stats-metric-label">Conversion rate</div>
            <div className="uz-stats-metric-value">{conversionRate}%</div>
            <div className="uz-stats-metric-sub">Зочлолтоос дуудлага руу</div>
          </div>
        </div>
      </div>

      {/* INFO */}
      <div className="uz-stats-info">
        <strong>ℹ️ Мэдээлэл:</strong>
        <ul>
          <li><b>Нийт зочлолт:</b> Аппыг нээсэн нийт удаа</li>
          <li><b>Хувийн зочин:</b> Тус тусдаа хэрэглэгч (давтагдахгүй)</li>
          <li><b>Conversion rate:</b> Зочин/дуудлага харьцаа</li>
          <li>Сүүлийн 30 хоногийн мэдээлэл хадгалагдана</li>
        </ul>
        {lastVisit && (
          <div style={{ marginTop: 8, color: 'var(--ink-soft)', fontSize: 12 }}>
            Сүүлд орсон: {new Date(lastVisit).toLocaleString('mn-MN')}
          </div>
        )}
      </div>
    </div>
...
/* STATS PANEL */
.uz-stats-panel { display:flex; flex-direction:column; gap:24px; }
.uz-stats-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:14px; }
.uz-stat-card { padding:22px 20px; border-radius:18px; color:#fff; box-shadow:0 8px 24px -10px rgba(0,0,0,.15); }
.uz-stat-blue { background:linear-gradient(135deg,#0a6cff,#06173a); }
.uz-stat-green { background:linear-gradient(135deg,#16a085,#0d6e5a); }
.uz-stat-gold { background:linear-gradient(135deg,#f1a814,#d99a00); }
.uz-stat-purple { background:linear-gradient(135deg,#7a5af0,#5a3fc0); }
.uz-stat-icon { font-size:30px; margin-bottom:6px; }
.uz-stat-value { font-size:34px; font-weight:800; letter-spacing:-.02em; line-height:1; margin-bottom:6px; }
.uz-stat-label { font-size:13px; font-weight:600; opacity:.9; }

.uz-stats-compare { background:var(--card); border:1px solid var(--line); border-radius:16px; padding:18px 22px; display:flex; align-items:center; gap:24px; }
.uz-stats-compare-item { display:flex; flex-direction:column; gap:4px; }
.uz-stats-compare-label { font-size:12px; color:var(--ink-soft); font-weight:600; }
.uz-stats-compare-value { font-size:28px; font-weight:800; color:var(--ink); }
.uz-stats-compare-arrow { font-size:24px; }
.uz-stats-compare-diff { margin-left:auto; font-size:24px; font-weight:800; }

.uz-stats-section { background:var(--card); border:1px solid var(--line); border-radius:16px; padding:22px 24px; }
.uz-stats-section-title { font-size:16px; font-weight:800; color:var(--ink); margin-bottom:18px; }

.uz-stats-chart { display:flex; align-items:end; gap:8px; height:180px; padding:10px 0; border-bottom:2px solid var(--line); }
.uz-stats-bar-wrap { flex:1; display:flex; flex-direction:column; align-items:center; gap:6px; height:100%; justify-content:end; }
.uz-stats-bar-count { font-size:11px; font-weight:700; color:var(--ink); margin-bottom:2px; }
.uz-stats-bar { width:100%; max-width:40px; background:linear-gradient(180deg,#0a6cff,#06173a); border-radius:6px 6px 0 0; min-height:4px; transition:all .3s; }
.uz-stats-bar-today { background:linear-gradient(180deg,#f1a814,#d99a00) !important; }
.uz-stats-bar-label { font-size:10px; color:var(--ink-soft); font-weight:600; margin-top:4px; }

.uz-stats-metrics { display:grid; grid-template-columns:repeat(auto-fit,minmax(180px,1fr)); gap:14px; }
.uz-stats-metric { background:#f4f6fa; border-radius:12px; padding:16px 18px; }
.uz-stats-metric-label { font-size:12px; color:var(--ink-soft); font-weight:600; margin-bottom:6px; }
.uz-stats-metric-value { font-size:30px; font-weight:800; color:var(--ink); line-height:1; margin-bottom:6px; letter-spacing:-.02em; }
.uz-stats-metric-sub { font-size:11px; color:var(--ink-soft); line-height:1.4; }

.uz-stats-info { background:#eef2ff; border:1px solid #d2dfff; border-radius:14px; padding:18px 22px; color:var(--ink); font-size:13px; }
.uz-stats-info ul { margin:8px 0 0; padding-left:20px; }
.uz-stats-info li { margin-bottom:4px; }
.uz-stats-info b { color:var(--blue); }

@media (max-width:560px){
  .uz-stats-grid { grid-template-columns:1fr 1fr; }
  .uz-stat-value { font-size:28px; }
  .uz-stats-compare { flex-wrap:wrap; gap:14px; padding:14px; }
  .uz-stats-compare-value { font-size:24px; }
  .uz-stats-section { padding:16px 18px; }
  .uz-stats-chart { height:140px; }
  .uz-stats-bar { max-width:24px; }
}
