...
export default function App() {
  ...
  async function markBookingPaid(bookingId) {
    const next = bookings.map(b => b.id === bookingId ? { ...b, paymentStatus: 'paid', status: 'Сантехникч хайж байна' } : b);
    setBookings(next); await saveList(BOOKINGS_KEY, next);
  ...
  async function submitFeedback(data) {
    ...
    setFeedback(next); await saveList(FEEDBACK_KEY, next);
    showToast('Таны санал амжилттай илгээгдлээ. Баярлалаа!');
  }
  async function markFeedbackRead(id) {
    const next = feedback.map(f => f.id === id ? { ...f, read: true } : f);
    setFeedback(next); await saveList(FEEDBACK_KEY, next);
  }
  ...
  return (
    <div className="uz">
      ...
      {mode === 'customer' && view === 'pay-booking' && payBookingId && (
        <BookingPayment booking={bookings.find(b => b.id === payBookingId)} settings={settings} onDone={() => { setPayBookingId(null); setView('mybookings'); }} />
      )}
      ...
      {mode === 'admin' && adminAuthed && (
        <AdminDashboard techs={techs} bookings={bookings} customers={customers} feedback={feedback} settings={settings} services={services} materials={materials} matOrders={matOrders} onToggleBlock={toggleBlock} onApproveTech={approveTech} onRejectTech={rejectTech} onSetArrival={setArrivalTime} onSetExtraWait={setExtraWait} onSaveSettings={saveSettings} onCreditTech={creditTech} onSaveServices={saveServices} onSaveMaterials={saveMaterials} onSendMessage={sendMessage} onDeleteBooking={deleteBooking} onMarkPaid={markBookingPaid} onMarkFeedbackRead={markFeedbackRead} onDeleteFeedback={deleteFeedback} onRefresh={refreshAll} />
      )}
...
// ============ HOME ============
function Home({ onPlaceCall, onMaterials, onTechRegister, onTrack, onAdmin, onHelp, onFeedback }) {
  return (
...
// ============ BOOKING PAYMENT ============
function BookingPayment({ booking, settings, onDone }) {
  ...
  const total = callFee - (booking.discountApplied || 0);
  return (
    <div className="uz-page" style={{ maxWidth: 560 }}>
      <div className="uz-pay-required">
        ...
        <p style={{ textAlign: 'center', color: 'var(--ink-soft)', marginBottom: 24, fontSize: 14 }}>
          ...
          Төлөх дүн: <b style={{ color: 'var(--ink)', fontSize: 22 }}>₮{fmt(total)}</b>
        ...
        <p style={{ textAlign: 'center', marginBottom: 18, fontSize: 14, lineHeight: 1.5 }}>
          Доорх данс руу <b>₮{fmt(total)}</b> шилжүүлээд, <b>гүйлгээний утганд дуудлагын дугаар ({booking.code})</b>-аа бичнэ үү. Төлбөр баталгаажсаны дараа сантехникч хайж эхлэх болно.
        </p>
        {hasBank ? (
          <div className="uz-bank-box uz-bank-box--lg" style={{ marginBottom: 18 }}>
            ...
            <div className="uz-bank-row"><span>Гүйлгээний утга:</span> <b>{booking.code}</b> <CopyButton text={booking.code} /></div>
    ...
    <div className={`uz-booking-card ${b.disputed ? 'uz-card-disputed' : ''}`}>
      ...
      <div className="uz-bc-body">
        ...
        {b.paymentStatus === 'pending' && (
          <div className="uz-pay-pending">
            ⏳ <b>Төлбөр хүлээгдэж байна.</b> Дуудлага идэвхжихийн тулд эхлээд төлбөрөө хийнэ үү.
          </div>
        )}
            ...
            <div key={b.id} className={`uz-booking-card ${b.disputed ? 'uz-card-disputed' : ''}`}>
              ...
              <div className="uz-bc-action" style={{ flexDirection: 'column', gap: 6 }}>
                {b.paymentStatus === 'pending' && onMarkPaid && (
                  <button className="uz-confirm-pay-btn" onClick={() => {
                    if (window.confirm(`${b.code} дуудлагын төлбөр орсон уу? Баталгаажуулсны дараа сантехникч харж эхэлнэ.`)) {
                      onMarkPaid(b.id);
                    }
                  }}>✓ Төлбөр орсон</button>
                )}
      ...
      {feedback.map(f => (
        <div key={f.id} className={`uz-feedback-card ${!f.read ? 'unread' : ''}`}>
          ...
          <div className="uz-feedback-actions">
            ...
            <button className="uz-del-btn" onClick={() => {
              if (window.confirm('Энэ саналыг устгах уу?')) onDelete(f.id);
            }}>🗑 Устгах</button>
          </div>
        </div>
      ))}
    ...
    <div className="uz-settings">
      ...
      <div className="uz-settings-form" style={{ marginBottom: 28 }}>
        <div className="uz-field"><label>📞 Үйлчлүүлэгчийн дуудлагын хураамж (₮)</label>
          <input type="number" value={calloutFee} onChange={e => setCalloutFeeState(e.target.value)} placeholder="5000" />
        ...
        <div className="uz-field"><label>👷 Сантехникчийн нэг дуудлагын хураамж (₮)</label>
          <input type="number" value={callFee} onChange={e => setCallFee(e.target.value)} placeholder="5000" />
        ...
        <div className="uz-field"><label>📝 Сантехникчийн бүртгэлийн нэг удаагийн хураамж (₮)</label>
          <input type="number" value={regFee} onChange={e => setRegFee(e.target.value)} placeholder="10000" />
      ...
      <div className="uz-settings-form" style={{ marginBottom: 28 }}>
        <div className="uz-field"><label>📘 Facebook хуудасны URL</label>
          <input value={fbLink} onChange={e => setFbLink(e.target.value)} placeholder="Жишээ: https://facebook.com/usanzasvar" /></div>
        <div className="uz-field"><label>💬 Messenger линк</label>
          <input value={messengerLink} onChange={e => setMessengerLink(e.target.value)} placeholder="Жишээ: https://m.me/usanzasvar" /></div>
        <div className="uz-field"><label>📱 Viber линк</label>
          <input value={viberLink} onChange={e => setViberLink(e.target.value)} placeholder="Жишээ: viber://chat?number=%2B97699112233" /></div>
      </div>
      ...
      <div className="uz-settings-form" style={{ marginBottom: 28 }}>
        <div className="uz-field"><label>Төлбөрийн линк (URL)</label>
          <input value={paymentLink} onChange={e => setPaymentLink(e.target.value)} placeholder="Жишээ: https://qpay.mn/.... эсвэл https://i.socialpay.mn/..." /></div>
      </div>
...
.uz-logo-text { font-family:'Manrope',sans-serif; font-weight:800; font-size:46px; color:#fff; letter-spacing:-.02em; line-height:1; }
...
.uz-section-title { font-family:'Manrope',sans-serif; font-weight:700; font-size:30px; letter-spacing:-.02em; }
...
.uz-page-title { font-family:'Manrope',sans-serif; font-weight:700; font-size:30px; letter-spacing:-.02em; margin-bottom:24px; }
...
.uz-bc-name { font-family:'Manrope',sans-serif; font-weight:700; font-size:17px; }
...
.uz-home-card-title { font-family:'Manrope',sans-serif; font-weight:800; font-size:30px; line-height:1.1; letter-spacing:-.02em; }
.uz-home-card-sub { font-size:16px; opacity:.92; margin-top:6px; }
@media (max-width:560px){ .uz-home-card-title { font-size:26px; } .uz-home-card-icon { font-size:56px; } }
...
.uz-confirm-pay-btn { background:var(--green); color:#fff; border:none; border-radius:9px; padding:10px 14px; font-weight:700; font-size:13px; cursor:pointer; white-space:nowrap; }
.uz-confirm-pay-btn:hover { background:#0d8a56; }
