...
export default function App() {
  ...
  // Notification: detect new bookings/changes
  ...
  useEffect(() => {
    ...
    if (mode === 'admin' && adminAuthed && newOnes.length > 0) {
      newOnes.forEach(b => {
        showBrowserNotification('🛎 Шинэ дуудлага!', `${b.serviceName} · ${b.district} · ${b.customerName || ''}`, { tag: 'new-booking-' + b.id });
      });
    ...
    if (mode === 'tech' && techPhone) {
      ...
      if (tech) {
        ...
        if (relevant.length > 0) {
          relevant.forEach(b => {
            showBrowserNotification('🔧 Шинэ дуудлага бэлэн!', `${b.serviceName} · ${b.district}`, { tag: 'tech-' + b.id });
          });
          playNotificationSound();
      ...
      bookings.forEach(b => {
        ...
        if (prev && prev !== current) {
          if (current === 'paid' && prev === 'pending') {
            showBrowserNotification('✅ Төлбөр баталгаажлаа!', 'Сантехникчид мэдээлэл илгээгдсэн.', { tag: 'paid-' + b.id });
            playNotificationSound();
          } else if (current === 'accepted') {
            showBrowserNotification('✓ Сантехникч хүлээж авлаа!', `${b.techName || 'Сантехникч'} тантай уулзах гэж явж байна.`, { tag: 'accepted-' + b.id });
            playNotificationSound();
          } else if (current === 'completed') {
            showBrowserNotification('✓ Дуудлага дууссан!', 'Үнэлгээ өгөхөө бүү март!', { tag: 'completed-' + b.id });
            playNotificationSound();
          }
        }
      ...
      {mode === 'tech' && view === 'techdash' && currentTech && (
        currentTech.blocked
          ...
          : currentTech.approved === false
            ...
            : <TechDashboard tech={currentTech} bookings={bookings} settings={settings} onAccept={acceptCall} onComplete={completeCall} onRefresh={refreshAll} onSendMessage={sendMessage} onReplyWarning={replyToWarning} onMarkWarningRead={markWarningRead} />
      )}
...
// ============ TECH DASHBOARD ============
function TechDashboard({ tech, bookings, settings, onAccept, onComplete, onRefresh, onSendMessage, onReplyWarning, onMarkWarningRead }) {
