...
export default function App() {
  ...
  async function createBooking(data) {
    ...
    if (useDiscount) {
      const updatedCustomers = {
        ...customers,
        [custPhone]: { coins: custCoins - coinsForDiscount }
      };
      setCustomers(updatedCustomers);
      await saveObj(CUSTOMERS_KEY, updatedCustomers);
    }
    setMyPhone(custPhone);
    showToast(useDiscount ? `Дуудлага бүртгэгдлээ! ₮${fmt(discountAmt)} хямдрал авлаа 🎁` : 'Дуудлага амжилттай бүртгэгдлээ');
    setView('mybookings');
  }
  async function rateBooking(bookingId, rating, onTime) {
    const techCoinAmt = (settings.techCoinPerComplete || 1);
    const custCoinAmt = (settings.customerCoinPerComplete || 1);
    let next = bookings.map(b => b.id === bookingId ? {
      ...b,
      rating,
      onTime,
      coinAwarded: onTime,
    ...
    if (b) {
      ...
      const updatedCustomers = {
        ...customers,
        [custPhone]: { coins: ((customers[custPhone] && customers[custPhone].coins) || 0) + custCoinAmt }
      };
      setCustomers(updatedCustomers);
      await saveObj(CUSTOMERS_KEY, updatedCustomers);
    }
  ...
  async function acceptCall(booking, payMethod) {
    ...
    await updateTech(techPhone, t => payMethod === 'money' ? { ...t, balance: t.balance - perCallFee } : { ...t, coins: t.coins - coinsForFree });
  ...
  async function completeCall(booking) {
    const next = bookings.map(b => b.id === booking.id ? { ...b, completed: true, status: 'Дууссан · үнэлгээ хүлээж байна' } : b);
    setBookings(next); await saveList(BOOKINGS_KEY, next);
  ...
  async function sendMessage(bookingId, from, text) {
    ...
    const next = bookings.map(b => b.id === bookingId ? { ...b, messages: [...(b.messages || []), msg] } : b);
    setBookings(next); await saveList(BOOKINGS_KEY, next);
    ...
    <div className="uz">
      ...
      {mode === 'customer' && view === 'home' && (
        <Home
          ...
          onTechRegister={() => { setMode('tech'); setView('techentry'); }}
      ...
      {mode === 'customer' && view === 'services' && (
        <ServicesPage services={services} settings={settings} onBook={(s) => { setActiveService(s); setView('booking'); }} onBack={() => setView('home')} />
      )}
      {mode === 'customer' && view === 'booking' && (
        <Booking service={activeService} customers={customers} settings={settings} onCancel={() => setView('services')} onSubmit={createBooking} />
      )}
      {mode === 'customer' && view === 'mybookings' && (
        <MyBookings bookings={myBookings} hasPhone={!!myPhone} onLookup={(p) => setMyPhone(cleanPhone(p))} onNew={() => setView('home')} onRate={rateBooking} onDispute={disputeBooking} onSendMessage={sendMessage} onBack={() => setView('home')} myCoins={myPhone && customers[myPhone] ? customers[myPhone].coins || 0 : 0} settings={settings} />
      )}
      ...
      {mode === 'tech' && view === 'techdash' && currentTech && (
        currentTech.blocked
          ...
          : currentTech.approved === false
            ...
            : <TechDashboard tech={currentTech} bookings={bookings} settings={settings} onAccept={acceptCall} onComplete={completeCall} onRefresh={refreshAll} onSendMessage={sendMessage} />
      )}

      {/* ADMIN */}
      {mode === 'admin' && !adminAuthed && (
        <AdminLogin onAuth={() => { setAdminAuthed(true); refreshAll(); }} onBack={() => { setMode('customer'); setView('home'); }} />
      )}
      {mode === 'admin' && adminAuthed && (
        <AdminDashboard techs={techs} bookings={bookings} customers={customers} settings={settings} services={services} materials={materials} matOrders={matOrders} onToggleBlock={toggleBlock} onApproveTech={approveTech} onRejectTech={rejectTech} onSetArrival={setArrivalTime} onSetExtraWait={setExtraWait} onSaveSettings={saveSettings} onCreditTech={creditTech} onSaveServices={saveServices} onSaveMaterials={saveMaterials} onSendMessage={sendMessage} onDeleteBooking={deleteBooking} onRefresh={refreshAll} />
      )}
...
// ============ HOME ============
function Home({ onPlaceCall, onMaterials, onTechRegister, onTrack, onAdmin, onHelp }) {
  return (
    <div className="uz-home">
      <div className="uz-home-header">
        <h1 className="uz-home-title">Сайн уу!</h1>
        <p className="uz-home-sub">Та юу хийхийг хүсэж байна?</p>
      </div>
      <div className="uz-home-cards">
        ...
        <button className="uz-home-card uz-home-card--mat" onClick={onMaterials}>
          <div className="uz-home-card-icon">🛠</div>
          <div className="uz-home-card-title">Материал</div>
...
function ServicesPage({ services, settings, onBook, onBack }) {
        ...
        <div className="uz-pay-required">
          ...
          {hasBank ? (
            <div className="uz-bank-box uz-bank-box--lg" style={{ marginBottom: 18 }}>
              ...
              <div className="uz-bank-row"><span>Гүйлгээний утга:</span> <b>{done.code}</b> <CopyButton text={done.code} /></div>
...
// ============ BOOKING ============
function Booking({ service, customers, settings, onCancel, onSubmit }) {
  const [district, setDistrict] = useState(DISTRICTS[0]);
  ...
  function submit() {
    ...
    setErr('');
    onSubmit({ serviceId: service.id, serviceName: service.name, serviceIcon: service.icon, priceMin: service.priceMin, priceMax: service.priceMax, district: district.name, eta: district.eta, customerName: custName.trim(), address: address.trim(), phone, when, schedTime, note: note.trim(), useDiscount: canDiscount && useDiscount });
  }
  return (
    <div className="uz-page">
        ...
        <aside className="uz-summary">
          <div className="uz-summary-card">
            ...
            <div className="uz-summary-total"><span>Нийт (ойролцоо)</span>
              <span className="uz-total-val">₮{fmt(service.priceMin + CALLOUT_FEE - (canDiscount && useDiscount ? discAmt : 0))}{service.priceMax ? `–${fmt(service.priceMax + CALLOUT_FEE - (canDiscount && useDiscount ? discAmt : 0))}` : '+'}</span></div>
...
// ============ MY BOOKINGS ============
function MyBookings({ bookings, hasPhone, onLookup, onNew, onRate, onDispute, onSendMessage, onBack, myCoins, settings }) {
  ...
  return (<div className="uz-page"><button className="uz-back" onClick={onBack}>← Нүүр буцах</button><h1 className="uz-page-title">Миний дуудлага</h1>
    ...
    <div className="uz-bookings">{bookings.map(b => <CustomerBookingCard key={b.id} b={b} onRate={onRate} onDispute={onDispute} onSendMessage={onSendMessage} />)}</div></div>);
}

function CustomerBookingCard({ b, onRate, onDispute, onSendMessage }) {
      ...
      <div className="uz-bc-body">
        ...
        {b.completed && !b.disputed && b.rating == null && (
          <div className="uz-rate-box-v2">
            ...
            <button className="uz-rate-submit" style={{ width: '100%', padding: '12px' }} disabled={!rating || ratingOnTime === null} onClick={() => onRate(b.id, rating, ratingOnTime)}>Үнэлгээ илгээх</button>
          </div>
        )}
        {b.rating != null && (
...
// ============ TECH DASHBOARD ============
function TechDashboard({ tech, bookings, settings, onAccept, onComplete, onRefresh, onSendMessage }) {
    ...
    <div className={`uz-booking-card uz-tech-call uz-tech-call--${type}`}>
      ...
      <div className="uz-bc-body">
        ...
        <div className="uz-tech-info-grid">
          <div className="uz-tech-info-item"><span>👤 Захиалагч:</span><b>{b.customerName}</b></div>
          <div className="uz-tech-info-item"><span>📞 Утас:</span><b>+976 {fmtPhone(b.phone)}</b></div>
          <div className="uz-tech-info-item"><span>📍 Дүүрэг:</span><b>{b.district}</b></div>
          <div className="uz-tech-info-item"><span>🏠 Хаяг:</span><b>{b.address}</b></div>
          <div className="uz-tech-info-item"><span>💵 Үнэ:</span><b>₮{fmt(b.priceMin)}{b.priceMax ? `–${fmt(b.priceMax)}` : '+'}</b></div>
...
function AdminDashboard({ techs, bookings, customers, settings, services, materials, matOrders, onToggleBlock, onApproveTech, onRejectTech, onSetArrival, onSetExtraWait, onSaveSettings, onCreditTech, onSaveServices, onSaveMaterials, onSendMessage, onDeleteBooking, onRefresh }) {
          ...
          <div className="uz-bookings">{bookings.map(b => (
            <div key={b.id} className={`uz-booking-card ${b.disputed ? 'uz-card-disputed' : ''}`}>
              ...
              <div className="uz-bc-action" style={{ flexDirection: 'column', gap: 6 }}>
                ...
                {onDeleteBooking && (
                  <button className="uz-del-btn" onClick={() => {
                    if (window.confirm(`Дуудлага ${b.code} -г үүрд устгах уу? Энэ үйлдлийг буцаах боломжгүй.`)) {
...
function BankSettings({ settings, onSave }) {
  ...
  return (
    <div className="uz-settings">
      ...
      <p className="uz-settings-sub">Сантехникчээс авах хураамжуудыг тохируулна. Өөрчилсний дараа шинэ бүртгэл, шинэ дуудлагад шууд тусна.</p>
      <div className="uz-settings-form" style={{ marginBottom: 28 }}>
        <div className="uz-field"><label>Бүртгэлийн нэг удаагийн хураамж (₮)</label>
          <input type="number" value={regFee} onChange={e => setRegFee(e.target.value)} placeholder="10000" /></div>
        <div className="uz-field"><label>Дуудлага бүрийн хураамж (₮)</label>
          <input type="number" value={callFee} onChange={e => setCallFee(e.target.value)} placeholder="1000" /></div>
      </div>
      ...
      <div className="uz-settings-form" style={{ marginBottom: 28 }}>
        <div className="uz-field"><label>Цагтаа ажилласан 1 дуудлагад өгөх coin</label>
          <input type="number" value={techCoinComp} onChange={e => setTechCoinComp(e.target.value)} placeholder="1" /></div>
        <div className="uz-field"><label>Үнэгүй дуудлага авах coin тоо</label>
          <input type="number" value={techCoinsFree} onChange={e => setTechCoinsFree(e.target.value)} placeholder="2" /></div>
      </div>
      ...
      <div className="uz-settings-form" style={{ marginBottom: 28 }}>
        <div className="uz-field"><label>1 дуудлагад өгөх coin</label>
          <input type="number" value={custCoinComp} onChange={e => setCustCoinComp(e.target.value)} placeholder="1" /></div>
        <div className="uz-field"><label>Хямдрал авах coin тоо</label>
          <input type="number" value={custCoinsDisc} onChange={e => setCustCoinsDisc(e.target.value)} placeholder="5" /></div>
        <div className="uz-field"><label>Хямдралын дүн (₮)</label>
          <input type="number" value={custDiscAmt} onChange={e => setCustDiscAmt(e.target.value)} placeholder="5000" /></div>
      </div>
    ...
    <div className="uz-modal-overlay" onClick={onClose}>
      <div className="uz-modal uz-modal-lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="uz-detail-grid">
          ...
          <div className="uz-detail-section">
            <h4>📋 Дуудлага</h4>
            <div className="uz-detail-row"><span>Үнэ:</span><b>₮{fmt(b.priceMin)}{b.priceMax ? `–${fmt(b.priceMax)}` : '+'}</b></div>
          ...
          <div className="uz-detail-section">
            ...
            {b.feeRefunded && <div className="uz-detail-row"><span>Буцаалт:</span><b style={{ color: 'var(--green)' }}>₮{fmt(CALLOUT_FEE)}</b></div>}
          </div>
...
.uz-logo-text { font-family:'Bricolage Grotesque',sans-serif; font-weight:800; font-size:46px; color:#fff; letter-spacing:-.02em; line-height:1; }
...
.uz-home { max-width:1140px; margin:0 auto; padding:48px 24px 60px; }
.uz-home-header { text-align:center; margin-bottom:40px; }
.uz-home-title { font-family:'Bricolage Grotesque',sans-serif; font-weight:800; font-size:clamp(32px,5vw,48px); letter-spacing:-.02em; line-height:1.1; margin-bottom:10px; }
.uz-home-sub { font-size:17px; color:var(--ink-soft); }
...
.uz-home-card--call { background:linear-gradient(135deg,var(--blue) 0%,var(--blue-dark) 100%); border-color:transparent; color:#fff; }
.uz-home-card--call:hover { box-shadow:0 28px 60px -24px rgba(10,108,255,.7); }
.uz-home-card--mat { background:linear-gradient(135deg,#16a085 0%,#0d8a6d 100%); border-color:transparent; color:#fff; }
.uz-home-card--mat:hover { box-shadow:0 28px 60px -24px rgba(22,160,133,.55); }
...
.uz-home-card-icon { font-size:64px; line-height:1; margin-bottom:14px; }
.uz-home-card-title { font-family:'Bricolage Grotesque',sans-serif; font-weight:800; font-size:24px; line-height:1.1; }
.uz-home-card-sub { font-size:14px; opacity:.85; }
...
.uz-coin-sec-title { font-family:'Bricolage Grotesque',sans-serif; font-weight:700; font-size:16px; margin:18px 0 10px; color:var(--ink); }
...
.uz-bank-row { display:flex; align-items:center; gap:8px; padding:6px 0; flex-wrap:wrap; }
.uz-bank-row span { color:var(--ink-soft); min-width:90px; display:inline-block; }
.uz-bank-row b { color:var(--ink); font-weight:700; flex:1; }

/* DELETE BOOKING BUTTON */
.uz-del-btn { background:var(--red-soft); color:var(--red); border:none; border-radius:9px; padding:8px 12px; font-weight:600; font-size:12px; cursor:pointer; white-space:nowrap; }
...
.uz-my-coins-num { font-family:'Bricolage Grotesque',sans-serif; font-weight:800; font-size:20px; line-height:1.2; }
...
.uz-help-num { width:38px; height:38px; border-radius:50%; background:var(--blue); color:#fff; display:flex; align-items:center; justify-content:center; font-family:'Bricolage Grotesque',sans-serif; font-weight:800; font-size:18px; flex-shrink:0; }
.uz-help-head h2 { font-family:'Bricolage Grotesque',sans-serif; font-size:21px; font-weight:700; letter-spacing:-.01em; }
...
.uz-qa-body { flex:1; }
.uz-qa-title { font-family:'Bricolage Grotesque',sans-serif; font-weight:800; font-size:22px; line-height:1.1; margin-bottom:4px; }
...
/* DETAIL BUTTON + MODAL */
.uz-detail-btn { background:var(--blue-soft); color:var(--blue); border:none; border-radius:9px; padding:10px 15px; font-weight:700; font-size:13px; cursor:pointer; white-space:nowrap; }
...
.uz-detail-section h4 { font-family:'Bricolage Grotesque',sans-serif; font-size:14px; font-weight:700; margin-bottom:10px; color:var(--ink); }
